...
// kiểm tra xem người dùng có phải client không 
...
// nén nội dung trong một thư mục thành một tệp zip
...
// giải nén tệp zip vào một thư mục 
...
// Căn giữa nội dung của tiêu đề và Menu
...
// hiển thị các tùy chọn và làm nổi bật mục hiện tại
...
// xử lý menu cho phép người dùng chọn các tùy chọn 
...
// hiển thị tên người dùng + đường dẫn đến vị trí hiện tại của người dùng trong thư mục  
char *handle_prompt(char *cur_user, char *user_dir) {
    const std::string app_name = "drive";
    std::vector<std::string> tokens = split(std::string(user_dir), '/');
    std::string cur_user_str(cur_user);

    // Tìm vị trí của người dùng trong đường dẫn thư mục user_dir
    // nếu tìm thấy lưu vị trí vào user_index 
    ...
    // tạo chuỗi prompt bao gồm: [userName@drive ~/]$
    ...
    // thêm đường dẫn đến vị trí hiện tại trong thư mục của người dùng 
    std::string path = "~/";
...
// loại bỏ các ký tự dư thừa trong một chuỗi (khoảng trắng, xuống dòng)
...
// tách tên người dùng từ đường dẫn cụ thể 
...
// yêu cầu người dùng nhập lệnh, loại bỏ các ký tự không mong muốn 
...
// tạo thư mục mới với quyền truy cập mặc định là 0755
...
// tạo một tệp rỗng mới nếu chưa tồn tại 
...
// thay đổi trạng thái khóa của tài khoản người dùng trong tệp ACCOUNTS_FILE
