...
// mã hóa mật khẩu người dùng bằng bảng băm 
...
// gửi thông tin (userName + passWord) đến server qua socket
// lưu userName vào cur_user nếu đăng nhập thành công 
int handle_login(int sockfd, char *cur_user) {
    Message msg;
    char *payload = (char *) malloc(PAYLOAD_SIZE);
    memset(payload, 0, PAYLOAD_SIZE);
    payload = get_acc_from_cli();   // nhập thông tin đang nhập username + password
...
// client gửi thông tin tài khoản mới đến server yêu cầu đăng ký tài khoản
int register_acc(int sockfd) {
    Message msg;
    char *payload = (char *) malloc(PAYLOAD_SIZE);
    memset(payload, 0, PAYLOAD_SIZE);
    payload = get_acc_from_cli();   // Nhâp userName + password
...
// kiểm tra userName + passWord so với file lưu trữ account 
// có cung cấp một số thao tác như khóa tài khoản, thay đổi thư mục làm việc của người dùng 
...
// kiểm tra tên đăng nhập đã tồn tại trong file account chưa 
...
// tạo tài khoản, tài khaonr được lưu vào file account 
// tạo thư mục cho tài khoản
int server_register(Message msg) {
    ...
    // nếu chưa => tạo tài khoản thêm vào file accounts
    ...
    // tạo thư mục chính cho người dùng 
